...
class HomePageView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        contex['local_news'] = News.published.all().filter(category__name="Uzbekistan").order_by('-publish_time')
        contex['world_news'] = News.published.all().filter(category__name="Jahon")[0:4]
        contex['sport_news'] = News.published.all().filter(category__name="Sport").order_by("-publish_time")
        contex['techno_news'] = News.published.all().filter(category__name="Fan_texnika").order_by("-publish_time")
        contex['economy'] = News.published.all().filter(category__name="Iqtisodiyot").order_by("-publish_time")
        # contex['galery'] = News.published.all().filter(category__name="Iqtisodiyot").order_by("-publish_time")[0:6]
        contex['jamiyat'] = News.published.all().filter(category__name="Jamiyat").order_by("-publish_time")
        return contex
